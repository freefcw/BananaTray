...
GNOME Shell Extension
  └─ QuotaClient -> BananaTrayIndicator -> Provider / quota overview
```
...
| 文件 | 说明 |
|------|------|
| `gnome-shell-extension/extension.js` | GNOME Shell 扩展入口和 UI 组件。只通过 `QuotaClient` 访问 D-Bus。 |
...
修改 `extension.js`、`stylesheet.css`、`metadata.json` 或 mock 数据后，关闭 nested Shell 并重新运行脚本。
...
- `extension.js` 不直接定义 D-Bus XML，不直接创建 proxy；协议层放在 `quotaClient.js`。
- `St.ScrollView` 使用 `set_child()`，不要使用 GNOME 50 下会崩的 `add_actor()`。
- 修改 UI 后优先在 nested Shell 中验证实际加载状态，而不是只看主会话。
- 新增扩展资产时同步 `scripts/check-gnome-extension.sh` 和安装说明，避免用户安装时漏复制子目录。
