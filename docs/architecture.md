...
- 当前开发构建依赖相邻目录中的 `../adabraka-gpui` 与 `../adabraka-ui` fork。`Cargo.toml` 直接使用这两个 path 依赖，并通过 `patch."https://github.com/freefcw/adabraka-gpui"` 让 `adabraka-ui` 内部的 GPUI 依赖解析到同一个本地 crate，避免出现两份不兼容的 GPUI 类型。
...
| Linux popup 设置 `WindowOptions.layer_shell`，同时保留窗口复用、拖动 saved position、透明渲染 + 鼠标穿透隐藏 | 优先在支持 `zwlr_layer_shell_v1` 的 Wayland compositor 上获得可控托盘弹窗定位；协议缺失或 X11/普通窗口路径下仍保留可用的拖动与复用兜底 | wlr-layer-shell 覆盖范围依赖桌面环境，GNOME 默认环境常会回落到 `xdg_toplevel`；普通 Wayland `xdg_toplevel` 不允许客户端指定窗口位置，`hide_window()`/`show_window()` 可能重新映射到屏幕中央，且 `start_window_move()` 期间可能产生失焦事件 | 当目标 Linux 桌面环境都支持稳定 layer-shell/ext-layer-shell popup，且 GPUI 能可靠处理 hide/show、焦点与 auto-hide 后，可删除普通窗口拖动保存和鼠标穿透隐藏兜底 |
