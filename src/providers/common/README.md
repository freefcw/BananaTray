...
- 覆盖 Cursor、Copilot、Devin（兼容旧 Windsurf 目录）/ Antigravity 这类本地配置扫描场景，保留 macOS `~/Library/Application Support` 与 XDG fallback 双路径扫描能力

### `http_client.rs` — HTTP 客户端（ureq）

为 API 类 Provider（Gemini, Custom YAML 等）提供统一的 HTTP 请求层：

- **`HttpError`** — 结构化 HTTP 错误枚举（`Timeout` / `Transport` / `HttpStatus { code }`），
...
- 全局共享 `LazyLock<Agent>`；`http_status_as_error(false)` 让 `check_status` 统一接管非 2xx，
  且错误正文**不读取、不记录、不传播**（可能含 token / 邮箱等敏感信息），`HttpStatus` 只携带状态码
...
providers/windsurf/       → http_client.rs（Devin / legacy Windsurf auth lookup。注：`windsurf` 目录与代码中各标识仍作为兼容稳定键/包名，对应界面上的 Devin）
