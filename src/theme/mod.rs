...
// ── YAML 主题加载 ────────────────────────────────────────
//
// 使用 serde_norway::Value 动态提取颜色值，无需 #[derive(Deserialize)]
...
pub(crate) fn color(root: &serde_norway::Value, section: &str, key: &str) -> Hsla {
...
fn load_theme(yaml: &str) -> Theme {
    let v: serde_norway::Value =
        serde_norway::from_str(yaml).expect("failed to parse theme YAML file");
